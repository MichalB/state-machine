...
class StateMachine extends EventEmitter {
  ...
  execute (client, input) {
    ...
    let eventName = this.getEvent ? this.getEvent.call(this, input, clientMeta) : input
    let timestamp = this.getTimestamp ? this.getTimestamp.call(this, input) : Date.now()
  ...
  transition (client, transition) {
    ...
    if (curStateObj.onExit) {
      result = result.then(() => clientMeta.inExitHandler = true)
      result = result.then(() => curStateObj.onExit.call(this, client, clientMeta.state))
      result = result.then(() => clientMeta.inExitHandler = false)
    }
    ...
    if (transition.onTransition) {
      result = result.then(() => transition.onTransition.call(this, client, clientMeta.state, newState))
    }

    // 4. Use new state
    result = result.then(() => clientMeta.state = newState)

    // 5. Call onEnter
    if (newStateObj.onEnter) {
      result = result.then(() => newStateObj.onEnter.call(this, client, clientMeta.state))
    }
